...
const Slider = () => {
  ...
  return (
    <div>
      <HiChevronLeft
        className="  hidden md:block text-white text-[30px] absolute mx-8 mt-[150px] cursor-pointer left-0"
        onClick={() => {
          sliderLeft(elementRef.current);
        }}
      />
      <HiChevronRight
        className=" hidden md:block text-[30px]  text-white absolute mx-8 mt-[150px] cursor-pointer right-0"
        onClick={() => {
    ...
    scrollbar-hide scroll-smooth"
      ...
      >
        {movieList.map((item, index) => (
          <img
            key={index} // Don't forget to add a unique key when mapping over elements
